...
TRANSFER_DESTINATION = os.getenv("TRANSFER_DESTINATION", "")  # ex: sip:Omnidrive@sip.rtc.elevenlabs.io:5060;transport=tcp

# SIP trunk (opcional - use se necessário)
...
async def dizparos_start_call(phone_e164: str) -> Dict[str, Any]:
    """
    Dispara uma chamada no Dizparos e transfere para o destino (ElevenLabs).
    Baseado no curl do painel do Dizparos.
    """
    if not DIZPAROS_TOKEN:
        raise RuntimeError("DIZPAROS_TOKEN não configurado.")
    if not TRANSFER_DESTINATION:
        raise RuntimeError("TRANSFER_DESTINATION não configurado (destino SIP do ElevenLabs).")
    ...
    # Se você precisar autenticar no SIP trunk, preencha as env vars.
...
async def tick(body: TickBody):
    """
    Processa campanhas 'running' e dispara chamadas para contatos 'pending',
    respeitando a concorrência definida em campaigns.concurrency.
    """
    sb = get_supabase()
...
async def dizparos_webhook(req: Request):
    """
    Recebe eventos do Dizparos:
    - 2000 answered
    - 2001 transferred
    - 2002 finished (com duration/cost/recording_url)
    """
    verify_webhook(req)
    ...
    if not diz_call_id:
        raise HTTPException(status_code=400, detail="call_id ausente no webhook")
    ...
    # se não achou a call ainda, não tem como atualizar
    if not call_rows:
